...
def init_schema(connection: sqlite3.Connection) -> None:
    ...
    connection.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_turns_source_active ON turns(source_key, active);
...
def upsert_turn(connection: sqlite3.Connection, turn: Turn) -> str:
    ...
    cursor = connection.execute(
        """
        INSERT INTO turns (
            turn_id, source_key, content_hash, source_msg_ids, conv_uuid,
            conv_title, ts, ordinal, human_text, assistant_text, full_text, active
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
        """,
        (
...
def search(
    ...
    *,
    ...
    if fts_terms:
        ...
        # Short-term-only query (e.g. "AI", "信仰"): trigram can't tokenize it, so
        # scan the turns table with LIKE. No bm25 without MATCH → rank by recency.
        # Recall over precision for short terms (spec §10 broad-net).
        from_clause = "turns t"
        score_expr = "0.0"
    ...
    # Short terms always go through LIKE (alongside FTS when present). AND-ed, so a
    # short term narrows the result set instead of zeroing it (no AND-poisoning).
